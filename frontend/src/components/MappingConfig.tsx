...
const SearchableSelect: React.FC<{
  ...
  placeholder: string;
  sourceField?: string; // Add sourceField prop for auto-matching
}> = ({ value, onChange, options, placeholder, sourceField }) => {
  ...
  const normalizeString = (str: string) => {
    return str.toLowerCase()
      .replace(/[\s_\[\]\.]/g, '') // Remove spaces, underscores, brackets, and dots
      .replace(/^.*payees\d+/, 'payee'); // Normalize array indices
  };

  // Auto-match fields when sourceField changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen && containerRef.current) {
      ...
      const dropdownHeight = 300; // maximum dropdown height
...
const MappingConfig: React.FC<MappingConfigProps> = ({
  ...
  useEffect(() => {
    if (mappings.length === 0 && headers.length > 0) {
      // Auto-create initial mappings for each CSV header
